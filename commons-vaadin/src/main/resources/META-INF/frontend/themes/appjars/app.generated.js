...
window.Vaadin['_vaadintheme_app_globalCss'] = window.Vaadin['_vaadintheme_app_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_app_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_app_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_app_globalCss'].push(target);
  }
